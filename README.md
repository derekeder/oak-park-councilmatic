...
docker compose run --rm app pupa init oak_park

# Run a scrape
docker compose run --rm app pupa update oak_park
```
...
- **SCSS files**: `oak_park/static/scss/`
- **JavaScript files**: `oak_park/static/js/`

Customize your city's branding by editing:
- `oak_park/static/scss/main.scss` - Colors, fonts, and styles
- `oak_park/static/js/main.js` - JavaScript functionality
...
adding templates to `oak_park/templates/`. 

### Models

Extend core models in `oak_park/models.py`:
```python
